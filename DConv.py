...
class DConv2d(nn.Module):
    __doc__ = r"""Applies 2D differential convolution over an input image composed of 
    several input plances.

    More about differential convolution at the end.

    Here, the notable parameter is `n_filters` which determines the number of output 
    channels for a given batch of inputs. First, the image is convoluted with a
    learnable filter and the appended with feature maps computed from Prewitt 
    operators for horizontal, vertical and diagonal edges. Other operations are 
    same as conventional Conv2d layer. 

    If a conv layer has n_filters as a parameter, then the output_channels is 
    5 * n_filters. Out of which input_channels * n_filters * kernel_size are learnable
    parameters.
    ...
    def _feature_maps(self, x : torch.tensor):
        ...
        for i in range(4):
            ...
            conv = F.conv2d(
                pad, 
                self._prewitt_op[i].repeat(self.n_filters, 1, 1, 1), 
